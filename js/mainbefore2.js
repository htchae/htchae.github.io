...
/**
 * Initialize animations using Intersection Observer for better performance
 */
function initAnimations() {
  ...
  const observer = new IntersectionObserver(
    ...
    {
      threshold: 0.1,
      rootMargin: '0px 0px 50px 0px', // Load slightly before becoming visible
    }
  );
...
/**
 * Initialize responsive carousel with optimized performance
 */
  ...
  function updateCarouselAnimation() {
    ...
    resizeDebounce = requestAnimationFrame(() => {
      ...
      if (firstImage.complete && firstImage.naturalWidth) {
        // Calculate dimensions once to avoid forced reflows
        ...
        requestAnimationFrame(() => {
          ...
          // Reset will-change after animation starts
          setTimeout(() => {
...
/**
 * Load the main carousel GIFs immediately with high priority
 */
function loadCarouselGifs() {
  ...
  carouselGifs.forEach((gif) => {
    ...
    if (src) {
      // Set high fetch priority for critical content
      const img = new Image();
...
/**
 * Add smooth touch scrolling behavior with performance optimization
//  */
// function initSmoothTouchScroll() {
...
//           // Apply some inertia to make scrolling feel smoother
//           if (Math.abs(diff) > 10) {
//             window.scrollBy({
//               top: -diff * 0.5,
...
// Project data array remains unchanged
